...
TEST(MaplePacketConstructorTest, one)
{
    ...
    MaplePacket pkt(0xAA, 0xBB, payload, 5);
    EXPECT_EQ(pkt.frameWord, 0xAABB0005);
...
{
    MaplePacket pkt(0x45, 0x89, 0xF8675309);
    EXPECT_EQ(pkt.frameWord, 0x45890001);
...
TEST(MaplePacketConstructorTest, three1)
{
    ...
    MaplePacket pkt(0x11111111, payload, 3);
    EXPECT_EQ(pkt.frameWord, 0x11111111);
    ...
    EXPECT_FALSE(pkt.isValid()); // len in frame word doesn't match size of vector
}

TEST(MaplePacketConstructorTest, three2)
{
    uint32_t payload[1] = {0x12345678};
    MaplePacket pkt(0x11111101, payload, 1);
    EXPECT_EQ(pkt.frameWord, 0x11111101);
    ASSERT_EQ(pkt.payload.size(), 1);
    EXPECT_EQ(pkt.payload[0], 0x12345678);
    EXPECT_TRUE(pkt.isValid());
}
...
TEST(MaplePacketConstructorTest, four2)
{
    ...
    EXPECT_EQ(pkt.frameWord, 0x65733fc7);
    ...
    EXPECT_FALSE(pkt.isValid()); // Len in frame does not match length of payload
}
...
TEST(MaplePacketConstructorTest, four3)
{
    ...
    MaplePacket pkt(words, 5);
    EXPECT_EQ(pkt.frameWord, 0x65733f04);
    ASSERT_EQ(pkt.payload.size(), 4);
    EXPECT_EQ(pkt.payload[0], 0x2c1bea03);
    EXPECT_EQ(pkt.payload[1], 0x70574afc);
    EXPECT_EQ(pkt.payload[2], 0xc720a39d);
...
TEST(MaplePacketCopyConstructorTest, one)
{
    ...
    EXPECT_EQ(pkt1, pkt2);
    EXPECT_EQ(pkt2.frameWord, 0x65733f04);
...
TEST(MaplePacketDefaultConstructorTest, constructAndSet)
{
    ...
    pkt.set(words, 3);
    EXPECT_EQ(pkt.frameWord, 0x59ef2302);
